  ...
  const contentMap = {
    Home: {
      0: {
        ...
        description:
          <p>Born in Mexico, I grew up in Wheaton, Illinois, with plans to move to Chicago. I have a passion for travel and hearing unique stories.<br/><br/>

My journey in technology began in middle school, and coding quickly became my favorite way to build and solve problems. I enjoy projects that blend technical skill with my artistic side—art is my creative outlet, enhancing my software development work. I love collaborating with other programmers and the rush of fixing a stubborn bug.<br/><br/>

I am deeply fascinated by cybersecurity, driven by the idea that system vulnerabilities can be exploited. This curiosity fuels my commitment to deeply understanding how computers operate and how to protect them.<br/><br/>

Committed to continuous learning and staying ahead of rapid technological change, I aim to use computers to impact others’ lives, one line of code at a time.</p>,
      },
    },
    Education: {
      0: {
    ...
    CyberSecurity: {
      0: { title: 'Project: BotNetGG', image: '/Projects/cyber/cyber1.png', description: "BotNet GoodGame lives up to its name: once a computer is infected, it's 'game over' for the victim, as I gain full control. This includes the ability to shut down the computer, simulate any key press (including typing), execute programs, place files in the startup menu, and create/run batch files. Multiple infected computers can be controlled simultaneously, executing the same command at a specified time, with a list presented for the attacker to choose from. What makes this impressive is the ability to send commands from anywhere in the world, transcending local network limitations. While typical computer control requires a local connection, this botnet allows full remote access as long as both computers are internet-connected. The source code can be provided upon request, but it will not be publicly released. The exploited vulnerability, common to most computers, will also remain undisclosed to prevent malicious use." },
      1: { title: 'Project: IamPopular.py', image: '/Projects/cyber/cyber2.png', description: "Inspired by Watch Dogs 2, ImPopular.py allows you to send simultaneous text messages to multiple phones in an area with a custom message anonymously. This creates a significant distraction, drawing everyone's attention to their devices and away from their surroundings. Written in Python, ImPopular uses datetime for timed attacks, selenium to identify phone carriers and linked email addresses, and the email library to send messages as texts. It takes an array of phone numbers, determines their carriers, and identifies associated emails. An SSH server on a Raspberry Pi awaits a signal from my iPhone to start an attack. Via iPhone app shortcuts, I can select targets, compose messages, and schedule the attack time." },
      2: { title: 'Project: Keylogger Around The World', image: '/Projects/cyber/cyber3.png', description: "This keylogger, disguised as a default Windows program and running silently in the background, ensures the user remains completely unaware of its presence. It records every keystroke to an encrypted .txt file, consuming minimal RAM to avoid detection even in a process list. After five Enter presses, the encrypted data is emailed to me for decryption, and the local file is reset. Without the decrypter, any discovered .txt file will be unreadable to the user." },
    },
    Games: {
      0: { title: 'Project: Connect 4', image: '/Projects/game/game1.png', description: <p>This Connect Four game is built entirely in Java using JavaFX for the interface and CSS for styling. The goal remains simple: be the first player to connect four discs of your color.

The application features three distinct scenes: an intro, the main game, and a "play again" screen that announces the winner. Moves are managed using a Stack, allowing players to undo turns, while a matrix tracks the colored discs on the board. The game offers three themes and a "start new game" option.

What makes this project truly interesting is the unit testing. Winning can occur in four directions: vertical, horizontal, ascending diagonal, and descending diagonal. I developed an efficient algorithm that tests every single possible victory condition across the entire board. This comprehensive testing approach verifies the game's victory detection logic perfectly and saves countless hours that would otherwise be spent manually playing and checking every potential winning scenario.<br/><br/><br/></p>},
